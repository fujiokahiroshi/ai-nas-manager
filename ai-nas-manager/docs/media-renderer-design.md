...
### 4.1.2 「クリックしても何も起きない」問題と設計の見直し(2026-08-31)

`render_choices`で表示した候補をクリックしても選択が反映されない、という報告を
受けて調査した結果、2つの根本原因が見つかった。ライブでの対話的デバッグ(ユーザーに
何度もクリックさせて都度サーバー状態を確認する)は非常に時間がかかり、ユーザーから
「設計を見直せ」と明確な指摘を受けた。単発の不具合修正ではなく、**この種の失敗
(タブが実は繋がっていない・応答していない)を仕組みとして検知できるように**設計を変更した。

**根本原因1: Private Network Access (PNA) プリフライトの未対応**
最近のChromium系ブラウザ(Edge/Chrome共)は、`file://`ページなどの「公開」コンテキストから
`127.0.0.1`のような「プライベート」アドレスへfetchする際、事前に`OPTIONS`プリフライト
リクエストを送り、サーバーが`Access-Control-Allow-Private-Network: true`ヘッダーで
明示的に許可しない限りfetch自体をブロックする(近年のブラウザ更新で有効化された仕様)。
`_Handler`は`do_OPTIONS`を実装しておらず、この機能が有効なブラウザでは
サーバー側のログには何も残らないまま、ブラウザ側で"TypeError: Failed to fetch"として
サイレントに失敗していた。`do_OPTIONS`を追加し、`_send_json`にも念のため同ヘッダーを
付与して解決。ブラウザの自動更新だけで突然発症しうる、原因の特定が難しい不具合。

**根本原因2: `webbrowser.open()`がOS既定ブラウザに丸投げ**
既定ブラウザが(ユーザーの環境変化で)Edgeから静かにChromeに変わっており、
`render_choices`が実際にはChromeでページを開いていたのに、ユーザー・Claude双方が
Edgeを見て「開いていない」と誤認していた。`_open_in_browser()`を新設し、
Edgeの実行ファイルを既知のパスから直接起動するようにして、既定ブラウザ設定に
依存しない・常に同じブラウザに開く動作に変更した(Edgeが見つからない場合のみ
`webbrowser.open()`にフォールバック)。

**設計変更: 「表示したつもり」を返さない**
上記2つとは別に、そもそも`play_channel`/`render_choices`/`render_picture`が
「タブを開いた(はず)」「既存タブに反映した(はず)」という**未検証の楽観的な
成功メッセージ**を返していたことが、問題の発覚を遅らせた最大の要因だった。
`_wait_for_ack()`を新設し、状態を書き込んだ後に対応するポーリングエンドポイント
(`/state`・`/choices`・`/picture`)へのGETが実際に届くまで(新規タブ最大6秒、
既存タブ再利用は最大2秒)待ってから結果を返すようにした。届かなければ
「応答が確認できませんでした」と正直に失敗を返す。これにより、今回のような
「サーバーは正常だがブラウザ側で何かが壊れている」状態を、Claudeが自分で
検知して次の一手(タブの再確認・再起動)に進めるようになった。

**根本原因3(ユーザー指摘により追加調査、2026-08-31): seqの取り違え**
ユーザーから「画面のevent情報と制御情報がうまくいっていない理由を検討しろ」と
指摘を受け、再調査して見つけた追加の欠陥。ブラウザ側は`data.seq !== lastSeq`
だけを見て再描画するかどうかを決めるが、リーダープロセスが再起動すると
サーバー側の`seq`は0から数え直される。ページ自体はリロードされないため、
ブラウザが保持している`lastSeq`(再起動前の値)は残ったまま。もし再起動後の
新しい`seq`が偶然この古い`lastSeq`と同じ値になると、ブラウザは「変化なし」と
誤判定し再描画をスキップしてしまう——サーバーとブラウザ、2つの独立したカウンタが
「プロセスが別物に入れ替わった」ことを検知できないまま、たまたま数値が
一致してしまう取り違えバグ。
→ プロセス起動時に一意な`INSTANCE_ID`(`uuid.uuid4().hex`)を生成し、
`/state`・`/choices`・`/picture`の応答すべてに含めるようにした。
各ページの`poll()`は`instance_id`が前回と変わっていれば、`seq`の値に
関係なく必ず再描画する。

**一般化(2026-08-31)**: ユーザーから、この種の不安定さは本プロジェクト固有ではなく
「MCPが繋がった状態でwindow/Chrome操作が行われる」構成一般に共通する普遍的な問題だと
指摘された(V8-CAD、Fusion 360をChrome経由で操作する場合でも同様の不安定さを確認)。
コントローラー(MCPサーバー)とウィンドウ/ビューアが別プロセス・別ライフサイクルで
動く以上、完全に無くすことはできない(分散システム一般の限界)。ただし
「サイレントに失敗したふりをする」ことは無くせる。今後この種の症状に当たったら、
以下の順で切り分ける:

1. コントローラー(サーバー)自体が生きて応答しているか、**ブラウザを介さない**
   クライアント(PowerShell/curl等)でまず確認する
2. コントローラーが「届いたはず」を検証せず成功を返していないか確認する
   (fire-and-forgetになっていないか。`_wait_for_ack`のような確認ステップの有無)
3. 同じリソース(ポート・ロック)を複数のコントローラープロセスが取り合っていないか
   (一部が黙って機能不全の「フォロワー」になっていないか)
4. ウィンドウ/ブラウザをOS既定の解決(`webbrowser.open()`等)に丸投げしていないか
   (既定アプリはユーザーの環境変化で静かに変わりうる。既知の実行ファイルを直接
   指定する方が安全)
5. クライアント側が「サーバーの入れ替わり」を検知できているか、それとも単なる
   カウンタの一致だけで判定していないか(`INSTANCE_ID`のような一意な識別子が必要)
6. ブラウザのセキュリティポリシー変更(Private Network Access等)が原因で
   ローカルネットワークへのリクエストがサイレントにブロックされていないか
7. ライブデバッグでユーザーに何度もクリックさせるのではなく、ページ読み込み時に
   自動でfetchして結果を報告する小さな自己完結テストページを作り、
   サーバー側・クライアント側を独立に検証する

**具体的な対策(2026-08-31、「対処をしておかないと時間ばかりかかる」との指摘を受けて実装)**:

- `diagnose_connection()`(MCPツール)を新設。上記1・2番を毎回手動のPowerShell
  フォレンジックをせずに、ツール1回の呼び出しで確認できるようにした。リーダーなら
  `instance_id`とstate/choices/pictureそれぞれの最終ポーリングからの経過秒数を、
  フォロワーならリーダーへの到達性を返す。
- 全ページ(player/chooser/picture)の右上(pictureのみ左下)に、接続状態を常時表示する
  小さなデバッグオーバーレイ(`DEBUG_OVERLAY_CSS`/`DEBUG_OVERLAY_JS`)を追加。
  直近のポーリングが成功していれば緑字で`OK <時刻> seq=<n> id=<instance_id先頭8桁>`、
  失敗していれば赤字で`ERR 応答なし`と最終成功時刻を表示する。Claudeに聞かなくても、
  画面を見るだけでユーザー自身が「サーバーと繋がっているか」を確認できる。
  実装時のハマりどころ: このオーバーレイの`<div>`も、4.1節の`<img>`と同じ理由
  (`position:fixed`の要素にwidth/heightを明示しないと描画されないことがある
  Chromium側の癖)で`width`/`height`を明示する必要があった。
