...
CHANNELS: list[MediaChannel] = [
    MediaChannel(
        ...
        fragments=[
            FragmentTag(0.0, 2.0, "センサー部品(ジャイロセンサーのサブアセンブリ)のクローズアップ。白背景で、組み立て前のパーツ紹介。"),
            FragmentTag(2.0, 18.2, "人の手が2輪のロボット本体を持ち、組み立て・調整をしている。"),
            FragmentTag(18.2, 46.7, "ロボットが人の手を離れて自立し、2輪だけでバランスを取りながら立っている(自己平衡のデモ)。"),
        ],
    ),
    MediaChannel(
        channel=2,
        ...
        fragments=[
            FragmentTag(0.0, 6.0, "アーム付きロボットが、4色のパーツが1つずつ入ったグラス4個の上に構えている引き画。"),
            FragmentTag(6.0, 10.6, "EV3画面のクローズアップ。右矢印アイコンと「# = 0」というカウンター表示。"),
            FragmentTag(10.6, 15.2, "最初とほぼ同じ引き画(内容重複、境界検出の再検出と見られる)。"),
            FragmentTag(15.2, 25.0, "人の手がEV3のボタンを押すクローズアップ。プログラム開始の操作。"),
            FragmentTag(25.0, 38.4, "アームが動作し、黄色いパーツをグラスへ投入する様子(モーションブラーあり)。"),
        ],
    ),
    MediaChannel(
        channel=3,
        ...
        fragments=[
            FragmentTag(0.0, 2.24, "ロボット頭部の極端なクローズアップ、斜めアングル(冒頭カット)。"),
            FragmentTag(2.24, 7.08, "ロボット全体の引き画、静止した状態。"),
            FragmentTag(7.08, 11.16, "直前とほぼ同一の引き画(内容重複、境界検出の再検出と見られる)。"),
            FragmentTag(11.16, 23.60, "境界検出されなかった区間。ロボットが静止、または大きな変化のない状態が続く。"),
            FragmentTag(23.60, 26.20, "手が画面に入ってくる(モーションブラーあり)。人の介入が始まる。"),
            FragmentTag(26.20, 28.36, "手によるパーツ操作の継続。"),
            FragmentTag(28.36, 33.72, "顔(液晶画面の表情表示部)の極端なクローズアップにカット。"),
            FragmentTag(33.72, 40.88, "引き画に戻り、ロボットが立ち姿勢からしゃがんだ姿勢に変化。"),
            FragmentTag(40.88, 45.00, "再び顔のクローズアップ、色味も変化。"),
            FragmentTag(45.00, 48.05, "引き画、しゃがみ姿勢のまま締めくくり。"),
        ],
    ),
    MediaChannel(
        channel=4,
        ...
        fragments=[
            FragmentTag(0.0, 5.9, "据え置き型のクレーン状アームと、対象物(黒い円盤)の引き画。"),
            FragmentTag(5.9, 8.3, "EV3画面のクローズアップ。「?」という疑問符のアイコン表示。"),
            FragmentTag(8.3, 19.1, "人の手がボタンを操作するクローズアップ。"),
            FragmentTag(19.1, 22.3, "再度手のクローズアップ、画面には引き続き「?」表示。"),
            FragmentTag(22.3, 31.4, "アームが動作(モーションブラーを伴う動き)する様子の引き画。"),
        ],
    ),
]
